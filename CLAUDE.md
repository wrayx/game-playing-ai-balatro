...
**The executor waits for the board to settle before returning.** Clicking Play or Discard returns while the game is still dealing: measured right after a discard, the hand read 7 cards for ~0.5s before the replacements landed. A state captured in that window shows a partial hand, and the tooltip sweep over it finds nothing — which is how an agent ends up reasoning about cards it could not read, and why an autonomous run logged `Matching 0 tooltips to 1 cards`. `_wait_until_settled` polls until the hand count repeats. It tracks the hand only; the score counter animates for longer, so a score read straight after a play can still be mid-count (one run read `'00'` for 296). Selection correction runs in both directions — clicking only the missing cards would leave a stray selection in place, and since a mismatch refuses the action, one leftover would deadlock the agent permanently.
...
**Reading the shop.** `entities.pair_with_prices` marks an item as for sale by the price tag above it, which is the only reliable discriminator: owned jokers and shop jokers share the `joker_card` class and are both on screen in the shop. The tag is drawn touching the card's top edge (measured at one pixel of overlap), so the pairing tolerates overlap — requiring a non-negative gap silently dropped most of the stock. `sweep_descriptions` hovers any detections, so the same machinery reads hand cards, owned jokers and shop stock; the tooltip is the only place the game states a joker's name, effect, rarity and stickers, and stickers are run-specific (`Perishable, 5 remaining`, `Rental, lose $3 at end of round`), so no external table can substitute for it. **Booster packs** can be bought and chosen from. Their contents share classes with the row you already own — a Buffoon pack offers `joker_card`s just like the jokers you hold — so they are separated by position: the owned row sits above `OWNED_ROW_MAX_FRACTION` of the window (measured at 0.11–0.12) and the offering below it (0.58–0.62). The pack's Select button has no UI class, so it is clicked relative to the chosen card rather than at a fixed window position, and the pack closing is what proves the choice landed.

One detection gap needs labelled data rather than code. **Vouchers are invisible** — the entities model has no voucher class, so a voucher in the shop is never detected and cannot be bought. The UI model also **misses the shop's Next Round and Reroll buttons in some states**: on two otherwise similar frames with the cursor clear, Next Round was found at 0.99 confidence with $7 in hand and not at all, at any threshold, with $1 — when Reroll is greyed out as unaffordable. Since buying usually leaves less than the $5 reroll price, that stranded the agent in the shop after every purchase. `ShopActionEngine.NEXT_ROUND_FALLBACK` clicks the button's layout position when detection fails, logging loudly; it is a workaround, and the fix is shop frames labelled in that state.

**What the agent is told, and why.** Prefer re-reading over remembering: the screen is the source of truth and cannot go stale, so anything still displayed is rendered into the prompt every turn rather than carried. Joker effects are the clearest case — swept from their tooltips and shown in full during play, because `Joker: joker_card` tells the model nothing about why it owns a Ride the Bus that only scales while it avoids face cards. Game *rules* are stated (interest is $1 per $5 held, capped at $5, so $25 is the inflection; scoring is chips × mult from hand level). Game *strategy* is deliberately not: which hands to favour by ante, when to hoard versus convert to jokers. Handing the model a human's conclusions makes it impossible to tell a capable agent from a well-briefed one. The one genuinely unreadable thing is the path to the current board, so `_blind_progress_summary` carries what has happened this blind — hands played, what each scored, discards spent — scoped by target score so a new blind starts a fresh list with no transition to detect.
